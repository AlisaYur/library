  ...
  class Datepicker extends Component {
    ...
    constructor(el, options) {
      ...
      if (!!options && options.hasOwnProperty('i18n') && typeof options.i18n
          === 'object') {
        this.options.i18n = $.extend({}, Datepicker.defaults.i18n,
            options.i18n);
      }
    ...
    gotoDate(date) {
      ...
      if (this.calendars) {
        let firstVisibleDate = new Date(this.calendars[0].year,
    ...
    render(year, month, randId) {
      ...
      let previousMonth = month === 0 ? 11 : month - 1,
          ...
          daysInPreviousMonth = Datepicker._getDaysInMonth(yearOfPreviousMonth,
              previousMonth);
      ...
      for (let i = 0, r = 0; i < cells; i++) {
        let day = new Date(year, month, 1 + (i - before)),
            ...
            isEndRange = opts.endRange && Datepicker._compareDates(
                opts.endRange, day),
            isInRange =
                opts.startRange && opts.endRange && opts.startRange < day && day
                < opts.endRange,
            isDisabled =
    ...
    renderHead(opts) {
      ...
      for (i = 0; i < 7; i++) {
        arr.push(
            `<th scope="col"><abbr title="${this.renderDayName(opts,
                i)}">${this.renderDayName(
                opts,
                i,
                true
            )}</abbr></th>`
    ...
    renderTitle(instance, c, year, month, refYear, randId) {
      ...
      for (arr = [], i = 0; i < 12; i++) {
        arr.push(
            ...
            ((isMinYear && i < opts.minMonth) || (isMaxYear && i
                > opts.maxMonth)
                ? 'disabled="disabled"'
                : '') +
            '>' +
