(function ($) {
  ...
  class Timepicker extends Component {
    ...
    _buildHoursView() {
      ...
      if (this.options.twelveHour) {
        for (let i = 1; i < 13; i += 1) {
          ...
          tick.css({
            left:
                this.options.dialRadius + Math.sin(radian) * radius
                - this.options.tickRadius + 'px',
            top:
                this.options.dialRadius - Math.cos(radian) * radius
                - this.options.tickRadius + 'px'
          });
          tick.html(i === 0 ? '00' : i);
        ...
        for (let i = 0; i < 24; i += 1) {
          ...
          let radian = i / 6 * Math.PI;
          let inner = i > 0 && i < 13;
          let radius = inner ? this.options.innerRadius
    ...
    _updateTimeFromInput() {
      // Get the time
      let value = ((this.el.value || this.options.defaultTime || '')
          + '').split(':');
    ...
    resetClock(delay) {
      let view = this.currentView,
          ...
          radian = value * unit,
          radius =
              isHours && value > 0 && value < 13 ? this.options.innerRadius
                  : this.options.outerRadius,
