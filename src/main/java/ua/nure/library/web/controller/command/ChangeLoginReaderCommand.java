 ...
 */
@Log4j
public class ChangeLoginReaderCommand implements Command {
  ...
  public String execute(final HttpServletRequest request, final HttpServletResponse response) {
    ...
    String ajaxFromRequest = request.getParameter(ajaxParamName);
    ...
    try {
      ...
      if (changeReader.getLogin().equalsIgnoreCase("")) {
      ...
      if (null != request.getSession(false) && null != request.getSession(false)
        ...
        String oldLogin = reader.getLogin();
        reader.setLogin(changeReader.getLogin());

        try {
          if (readerRepository.checkUniqueLoginReader(reader)) {
            readerRepository.updateAllReaderOrdersWithNewLogin(reader, oldLogin);
